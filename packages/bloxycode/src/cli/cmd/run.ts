...
export const RunCommand = cmd({
  command: "run [message..]",
  describe: "run bloxycode with a message",
  builder: (yargs: Argv) => {
    return yargs
      .positional("message", {
      ...
      .option("attach", {
        type: "string",
        describe: "attach to a running bloxycode server (e.g., http://localhost:4096)",
      })
      .option("port", {
        type: "number",
  ...
  handler: async (args) => {
    ...
    if (args.attach) {
      ...
      if (cfgResult.data && (cfgResult.data.share === "auto" || Flag.BLOXYCODE_AUTO_SHARE || args.share)) {
