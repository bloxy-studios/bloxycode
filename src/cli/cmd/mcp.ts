    ...
    await Instance.provide({
      ...
      async fn() {
        ...
        if (servers.length === 0) {
          ...
          prompts.outro("Add servers with: bloxycode mcp add")
          return
        }
    ...
    await Instance.provide({
      ...
      async fn() {
        ...
        if (oauthServers.length === 0) {
          ...
          prompts.log.info("Remote MCP servers support OAuth by default. Add a remote server in bloxycode.json:")
          prompts.log.info(`
  "mcp": {
    "my-server": {
      "type": "remote",
...
async function resolveConfigPath(baseDir: string, global = false) {
  // Check for existing config files (prefer .jsonc over .json, check .bloxycode/ subdirectory too)
  const candidates = [path.join(baseDir, "bloxycode.json"), path.join(baseDir, "bloxycode.jsonc")]

  if (!global) {
    candidates.push(path.join(baseDir, ".bloxycode", "bloxycode.json"), path.join(baseDir, ".bloxycode", "bloxycode.jsonc"))
  }
  ...
  // Default to bloxycode.json if none exist
  return candidates[0]
}
