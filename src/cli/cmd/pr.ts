...
export const PrCommand = cmd({
  command: "pr <number>",
  describe: "fetch and checkout a GitHub PR branch, then run bloxycode",
  builder: (yargs) =>
      ...
      async fn() {
        ...
        if (prInfoResult.exitCode === 0) {
          ...
          if (prInfoText.trim()) {
            ...
            // Check for bloxycode session link in PR body
            if (prInfo && prInfo.body) {
              const sessionMatch = prInfo.body.match(/https:\/\/opncd\.ai\/s\/([a-zA-Z0-9_-]+)/)
              if (sessionMatch) {
                const sessionUrl = sessionMatch[0]
                UI.println(`Found bloxycode session: ${sessionUrl}`)
                UI.println(`Importing session...`)

                const importResult = await $`bloxycode import ${sessionUrl}`.nothrow()
        ...
        UI.println()
        UI.println("Starting bloxycode...")
        UI.println()

        // Launch bloxycode TUI with session ID if available
        const { spawn } = await import("child_process")
        const bloxycodeArgs = sessionId ? ["-s", sessionId] : []
        const bloxycodeProcess = spawn("bloxycode", bloxycodeArgs, {
          stdio: "inherit",
          cwd: process.cwd(),
        })

        await new Promise<void>((resolve, reject) => {
          bloxycodeProcess.on("exit", (code) => {
            if (code === 0) resolve()
            else reject(new Error(`bloxycode exited with code ${code}`))
          })
          bloxycodeProcess.on("error", reject)
        })
      },
    })
  },
