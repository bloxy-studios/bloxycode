...
export const UninstallCommand = {
  command: "uninstall",
  describe: "uninstall bloxycode and remove all related files",
  builder: (yargs: Argv) =>
    yargs
      .option("keep-config", {
        alias: "c",
  ...
  handler: async (args: UninstallArgs) => {
    ...
    UI.println(UI.logo("  "))
    UI.empty()
    prompts.intro("Uninstall BloxyCode")
...
async function executeUninstall(method: Installation.Method, targets: RemovalTargets) {
  ...
  if (method !== "curl" && method !== "unknown") {
    ...
    if (cmd) {
      ...
      const result =
        method === "choco"
          ? await $`echo Y | choco uninstall bloxycode -y -r`.quiet().nothrow()
  ...
  UI.empty()
  prompts.log.success("Thank you for using BloxyCode!")
}
...
async function cleanShellConfig(file: string) {
  ...
  for (const line of lines) {
    ...
    if (skip) {
      skip = false
      if (trimmed.includes(".bloxycode/bin") || trimmed.includes("fish_add_path")) {
        continue
      }
    }

    if (
      (trimmed.startsWith("export PATH=") && trimmed.includes(".bloxycode/bin")) ||
      (trimmed.startsWith("fish_add_path") && trimmed.includes(".bloxycode"))
    ) {
      continue
    }
